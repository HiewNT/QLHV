...
public class HocVienCamController : Controller
{
    private readonly QlhvContext _context;

    public HocVienCamController(QlhvContext context)
    {
        _context = context;
    }
    ...
    public async Task<IActionResult> Details(string id)
    {
        ...
        var hocVien = await _context.Nguois
            ...
            .Include(h => h.KhenThuongNguois)
            .Include(h => h.KyLuatNguois)
            .FirstOrDefaultAsync(m => m.MaNguoi == id && m.LoaiNguoi == "HocVien" && m.QuocTich == "Campuchia");

        if (hocVien == null) return NotFound();
        return View(hocVien);
    }

    // GET: HocVienCam/Create
    ...
    public async Task<IActionResult> Create(Nguoi hocVien)
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            TempData["Success"] = "Thêm học viên Campuchia thành công!";
    ...
    // GET: HocVienCam/Edit/5
    public async Task<IActionResult> Edit(string id)
    {
        if (id == null) return NotFound();

        var hocVien = await _context.Nguois
            .FirstOrDefaultAsync(h => h.MaNguoi == id && h.LoaiNguoi == "HocVien" && h.QuocTich == "Campuchia");
        if (hocVien == null) return NotFound();
    ...
    public async Task<IActionResult> Edit(string id, Nguoi hocVien)
    {
        ...
        if (ModelState.IsValid)
        {
            try
            {
                ...
                await _context.SaveChangesAsync();
                TempData["Success"] = "Cập nhật học viên Campuchia thành công!";
            }
    ...
    public async Task<IActionResult> DeleteConfirmed(string id)
    {
        ...
        if (hocVien != null)
        {
            ...
            await _context.SaveChangesAsync();
            TempData["Success"] = "Xóa học viên Campuchia thành công!";
        }
        return RedirectToAction(nameof(Index));
    }
    ...
    {
        return _context.Nguois.Any(e => e.MaNguoi == id && e.LoaiNguoi == "HocVien" && e.QuocTich == "Campuchia");
    }
